<div class="front_page_section front_page_section_testimonials<?php
	$thegig_scheme = thegig_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $thegig_scheme ) && ! thegig_is_inherit( $thegig_scheme ) ) {
		echo ' scheme_' . esc_attr( $thegig_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( thegig_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( thegig_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$thegig_css      = '';
		$thegig_bg_image = thegig_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$thegig_anchor_icon = thegig_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$thegig_anchor_text = thegig_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( thegig_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$thegig_css      = '';
			$thegig_bg_mask  = thegig_get_theme_option( 'front_page_testimonials_bg_mask' );
			$thegig_bg_color_type = thegig_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $thegig_bg_color_type ) {
				$thegig_bg_color = thegig_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$thegig_caption = thegig_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $thegig_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $thegig_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $thegig_caption, 'thegig_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$thegig_description = thegig_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $thegig_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $thegig_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $thegig_description ), 'thegig_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! thegig_exists_trx_addons() ) {
						...
						thegig_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
