...
if ( ! function_exists( 'thegig_storage_get' ) ) {
	/**
	 ...
	 */
	function thegig_storage_get( $var_name, $default = '' ) {
		global $THEGIG_STORAGE;
		return isset( $THEGIG_STORAGE[ $var_name ] ) ? $THEGIG_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'thegig_storage_set' ) ) {
	/**
	 ...
	 */
	function thegig_storage_set( $var_name, $value ) {
		global $THEGIG_STORAGE;
		$THEGIG_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'thegig_storage_empty' ) ) {
	/**
	 ...
	 */
	function thegig_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $THEGIG_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $THEGIG_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $THEGIG_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $THEGIG_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'thegig_storage_isset' ) ) {
	/**
	 ...
	 */
	function thegig_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $THEGIG_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $THEGIG_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $THEGIG_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $THEGIG_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'thegig_storage_unset' ) ) {
	/**
	 ...
	 */
	function thegig_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $THEGIG_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $THEGIG_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $THEGIG_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $THEGIG_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'thegig_storage_inc' ) ) {
	/**
	 ...
	 */
	function thegig_storage_inc( $var_name, $value = 1 ) {
		global $THEGIG_STORAGE;
		if ( empty( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = 0;
		}
		$THEGIG_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'thegig_storage_concat' ) ) {
	/**
	 ...
	 */
	function thegig_storage_concat( $var_name, $value ) {
		global $THEGIG_STORAGE;
		if ( empty( $THEGIG_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'thegig_storage_get_array' ) ) {
	/**
	 ...
	 */
	function thegig_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $THEGIG_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $THEGIG_STORAGE[ $var_name ][ $key ] ) ? $THEGIG_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $THEGIG_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $THEGIG_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'thegig_storage_set_array' ) ) {
	/**
	 ...
	 */
	function thegig_storage_set_array( $var_name, $key, $value ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$THEGIG_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'thegig_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function thegig_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $THEGIG_STORAGE[ $var_name ][ $key ] ) ) {
			$THEGIG_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$THEGIG_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$THEGIG_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'thegig_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function thegig_storage_merge_array( $var_name, $key, $value ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$THEGIG_STORAGE[ $var_name ] = array_merge( $THEGIG_STORAGE[ $var_name ], $value );
		} else {
			$THEGIG_STORAGE[ $var_name ][ $key ] = array_merge( $THEGIG_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'thegig_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function thegig_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			thegig_array_insert_after( $THEGIG_STORAGE[ $var_name ], $after, $key );
		} else {
			thegig_array_insert_after( $THEGIG_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'thegig_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function thegig_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			thegig_array_insert_before( $THEGIG_STORAGE[ $var_name ], $before, $key );
		} else {
			thegig_array_insert_before( $THEGIG_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'thegig_storage_push_array' ) ) {
	/**
	 ...
	 */
	function thegig_storage_push_array( $var_name, $key, $value ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $THEGIG_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $THEGIG_STORAGE[ $var_name ][ $key ] ) ) {
				$THEGIG_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $THEGIG_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'thegig_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function thegig_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $THEGIG_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $THEGIG_STORAGE[ $var_name ] ) && is_array( $THEGIG_STORAGE[ $var_name ] ) && count( $THEGIG_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $THEGIG_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $THEGIG_STORAGE[ $var_name ][ $key ] ) && is_array( $THEGIG_STORAGE[ $var_name ][ $key ] ) && count( $THEGIG_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $THEGIG_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'thegig_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function thegig_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( empty( $THEGIG_STORAGE[ $var_name ][ $key ] ) ) {
			$THEGIG_STORAGE[ $var_name ][ $key ] = 0;
		}
		$THEGIG_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'thegig_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function thegig_storage_concat_array( $var_name, $key, $value ) {
		global $THEGIG_STORAGE;
		if ( ! isset( $THEGIG_STORAGE[ $var_name ] ) ) {
			$THEGIG_STORAGE[ $var_name ] = array();
		}
		if ( empty( $THEGIG_STORAGE[ $var_name ][ $key ] ) ) {
			$THEGIG_STORAGE[ $var_name ][ $key ] = '';
		}
		$THEGIG_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'thegig_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function thegig_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $THEGIG_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $THEGIG_STORAGE[ $var_name ] ) ? $THEGIG_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $THEGIG_STORAGE[ $var_name ] ) ? $THEGIG_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'thegig_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function thegig_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $THEGIG_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $THEGIG_STORAGE[ $var_name ]->$prop ) ? $THEGIG_STORAGE[ $var_name ]->$prop : $default;
	}
}
