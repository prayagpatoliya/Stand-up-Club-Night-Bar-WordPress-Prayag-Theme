...
if ( ! function_exists( 'thegig_array_get_first' ) ) {
	/**
	 ...
	 */
	function thegig_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'thegig_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function thegig_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'thegig_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function thegig_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'thegig_array_from_list' ) ) {
	/**
	 ...
	 */
	function thegig_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'thegig_array_slice' ) ) {
	/**
	 ...
	 */
	function thegig_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'thegig_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * thegig_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * thegig_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function thegig_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'thegig_array_insert_after' ) ) {
	/**
	 ...
	 */
	function thegig_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = thegig_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: thegig_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'thegig_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function thegig_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'thegig_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function thegig_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'thegig_generate_id' ) ) {
	/**
	 ...
	 */
	function thegig_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'thegig_get_tag' ) ) {
	/**
	 ...
	 */
	function thegig_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'thegig_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function thegig_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'thegig_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function thegig_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( thegig_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'thegig_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function thegig_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'thegig_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function thegig_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'thegig_get_column_class' ) ) {
	/**
	 ...
	 */
	function thegig_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'thegig_stripslashes' ) ) {
	/**
	 ...
	 */
	function thegig_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'thegig_get_value_gp' ) ) {
	/**
	 ...
	 */
	function thegig_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'thegig_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function thegig_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'thegig_get_value_gps' ) ) {
	/**
	 ...
	 */
	function thegig_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'thegig_get_session_value' ) ) {
	/**
	 ...
	 */
	function thegig_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'thegig_set_session_value' ) ) {
	/**
	 ...
	 */
	function thegig_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'thegig_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function thegig_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'thegig_get_cookie' ) ) {
	/**
	 ...
	 */
	function thegig_get_cookie( $name, $defa = '' ) {
		return thegig_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'thegig_set_cookie' ) ) {
	/**
	 ...
	 */
	function thegig_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'thegig_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//thegig_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//thegig_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'thegig' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'thegig_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'thegig_hex2rgba' ) ) {
	/**
	 ...
	 */
	function thegig_hex2rgba( $hex, $alpha ) {
		$rgb = thegig_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'thegig_hex2hsb' ) ) {
	/**
	 ...
	 */
	function thegig_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = thegig_rgb2hsb( thegig_hex2rgb( $hex ) );
...
if ( ! function_exists( 'thegig_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'thegig_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'thegig_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'thegig_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function thegig_hsb2hex( $hsb ) {
		return thegig_rgb2hex( thegig_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'thegig_date_to_sql' ) ) {
	/**
	 ...
	 */
	function thegig_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'thegig_format_price' ) ) {
	/**
	 ...
	 */
	function thegig_format_price( $price ) {
...
if ( ! function_exists( 'thegig_num2size' ) ) {
	/**
	 ...
	 */
	function thegig_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'thegig_size2num' ) ) {
	/**
	 ...
	 */
	function thegig_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'thegig_parse_num' ) ) {
	/**
...
if ( ! function_exists( 'thegig_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function thegig_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'thegig_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function thegig_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'thegig_is_on' ) ) {
	/**
...
if ( ! function_exists( 'thegig_is_off' ) ) {
	/**
...
if ( ! function_exists( 'thegig_is_inherit' ) ) {
	/**
	 ...
	 */
	function thegig_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'thegig_strshort' ) ) {
	/**
	 ...
	 */
	function thegig_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = thegig_strip_tags( $str );
...
if ( ! function_exists( 'thegig_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function thegig_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'thegig_strip_tags' ) ) {
	/**
	 ...
	 */
	function thegig_strip_tags( $str ) {
...
if ( ! function_exists( 'thegig_excerpt' ) ) {
	/**
	 ...
	 */
	function thegig_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return thegig_strwords( thegig_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'thegig_unserialize' ) ) {
	/**
	 ...
	 */
	function thegig_unserialize( $str ) {
...
if ( ! function_exists( 'thegig_str_replace' ) ) {
	/**
	 ...
	 */
	function thegig_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = thegig_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( thegig_str_replace( $from, $to, thegig_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'thegig_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function thegig_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'thegig_tags_count' ) ) {
	/**
	 ...
	 */
	function thegig_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'thegig_getimagesize' ) ) {
	/**
	 ...
	 */
	function thegig_getimagesize( $url, $echo = false ) {
		$img_size = false;
		$img_path = thegig_is_url( $url ) ? thegig_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'thegig_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function thegig_clear_thumb_size( $url, $remove_protocol = true ) {
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = thegig_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'thegig_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function thegig_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = thegig_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = thegig_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = thegig_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = thegig_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'thegig_get_thumb_size' ) ) {
	 ...
	 */
	function thegig_get_thumb_size( $ts ) {
		$retina = thegig_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'thegig_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'thegig_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'thegig-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'thegig_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'thegig-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'thegig_get_closest_thumb_size' ) ) {
	 ...
	 */
	function thegig_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'thegig_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function thegig_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = thegig_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'thegig_get_post_image' ) ) {
	/**
	 ...
	 */
	function thegig_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'thegig_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function thegig_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = thegig_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = thegig_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = thegig_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( thegig_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = thegig_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'thegig_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function thegig_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'thegig_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'thegig_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function thegig_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = thegig_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = thegig_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'thegig_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'thegig_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function thegig_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'thegig_get_post_video_list_first' ) ) {
	/**
...
if ( ! function_exists( 'thegig_get_post_iframe' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function thegig_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = thegig_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = thegig_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'thegig_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'thegig_get_embed_video' ) ) {
	/**
	 ...
	 */
	function thegig_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			// $embed_video = thegig_make_video_autoplay( $embed_video );
		} else if ( thegig_is_from_uploads( $video ) ) {
			...
			$video = thegig_add_to_url( $video, thegig_get_embed_video_parameters() );
			$tag   = thegig_get_embed_video_tag_name();
			// Calc video width and height (with ratio 16:9)
			$ratio  = explode( ':', apply_filters( 'thegig_filter_video_ratio', '16:9' ) );
			$width  = thegig_get_content_width();
			$height = round( $width / $ratio[0] * $ratio[1] );
			$dim    = apply_filters( 'thegig_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									) );
...
if ( ! function_exists( 'thegig_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function thegig_get_embed_video_tag_name() {
...
if ( ! function_exists( 'thegig_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function thegig_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = thegig_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										thegig_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'thegig_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function thegig_is_from_uploads( $url ) {
		$url          = thegig_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = thegig_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'thegig_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function thegig_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'thegig_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function thegig_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'thegig_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'thegig_init_filesystem', 0 );
	/**
	 ...
	 */
	function thegig_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'thegig_fpc' ) ) {
	 ...
	 */
	function thegig_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( thegig_is_on( thegig_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'thegig' ), $file ) );
					}
				}
			}
		}
	...
	function thegig_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? thegig_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( thegig_is_on( thegig_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'thegig' ), $file ) );
					}
				}
			}
		}
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( thegig_is_on( thegig_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'thegig' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'thegig_mkdir' ) ) {
	/**
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( thegig_is_on( thegig_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'thegig' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( thegig_is_on( thegig_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'thegig' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'thegig_unlink' ) ) {
	/**
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function thegig_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( thegig_is_on( thegig_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'thegig' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'thegig_copy' ) ) {
	/**
	...
	function thegig_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = thegig_prepare_path( $src );
				$dst = thegig_prepare_path( $dst );
...
if ( ! function_exists( 'thegig_unzip_file' ) ) {
	/**
...
if ( ! function_exists( 'thegig_esc' ) ) {
	 ...
	 */
	function thegig_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'thegig_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'thegig_check_min_file' ) ) {
	 ...
	 */
	function thegig_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && thegig_is_off( thegig_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && thegig_is_off( thegig_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'thegig_get_file_dir' ) ) {
	/**
	 ...
	 */
	function thegig_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! THEGIG_ALLOW_SKINS && ! thegig_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'thegig_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( THEGIG_CHILD_DIR != THEGIG_THEME_DIR && file_exists( THEGIG_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? THEGIG_CHILD_URL : THEGIG_CHILD_DIR ) . thegig_check_min_file( $file, THEGIG_CHILD_DIR );
			} elseif ( file_exists( THEGIG_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? THEGIG_THEME_URL : THEGIG_THEME_DIR ) . thegig_check_min_file( $file, THEGIG_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'thegig_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function thegig_get_file_url( $file ) {
		return thegig_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'thegig_get_file_ext' ) ) {
	/**
	 ...
	 */
	function thegig_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'thegig_get_file_name' ) ) {
	/**
	 ...
	 */
	function thegig_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'thegig_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function thegig_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'thegig_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( THEGIG_CHILD_DIR != THEGIG_THEME_DIR && is_dir( THEGIG_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? THEGIG_CHILD_URL : THEGIG_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( THEGIG_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? THEGIG_THEME_URL : THEGIG_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'thegig_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'thegig_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function thegig_get_folder_url( $folder ) {
		return thegig_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'thegig_merge_js' ) ) {
	/**
	 ...
	 */
	function thegig_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= thegig_fgc( thegig_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = thegig_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = thegig_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			thegig_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'thegig' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'thegig_filter_js_output', apply_filters( 'thegig_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'thegig_merge_css' ) ) {
	/**
	 ...
	 */
	function thegig_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'thegig_filter_responsive_sizes', thegig_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = thegig_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = thegig_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'thegig_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'thegig_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'thegig' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'thegig' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = thegig_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			thegig_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'thegig') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'thegig_filter_css_output', apply_filters( 'thegig_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'thegig_filter_merge_css' ) ) {
	add_filter( 'thegig_filter_merge_css', 'thegig_filter_merge_css' );
	/**
	 ...
	 */
	function thegig_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'thegig_merge_sass' ) ) {
	/**
	 ...
	 */
	function thegig_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'thegig_filter_responsive_sizes', thegig_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'thegig_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			thegig_fpc( thegig_get_file_dir( $to ), apply_filters( 'thegig_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'thegig_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function thegig_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'thegig_add_protocol' ) ) {
	/**
	 ...
	 */
	function thegig_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? thegig_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'thegig_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function thegig_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'thegig_add_to_url' ) ) {
	/**
	 ...
	 */
	function thegig_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = thegig_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'thegig_is_url' ) ) {
	/**
	 ...
	 */
	function thegig_is_url( $url ) {
		return strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false;
	}
}

if ( ! function_exists( 'thegig_is_external_url' ) ) {
	/**
	 ...
	 */
	function thegig_is_external_url( $url ) {
		return thegig_is_url( $url ) && strpos( $url, thegig_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}

if ( ! function_exists( 'thegig_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function thegig_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => thegig_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => THEGIG_CHILD_DIR,
				'url' => thegig_remove_protocol_from_url( THEGIG_CHILD_URL )
				),
			'theme' => array(
				'dir' => THEGIG_THEME_DIR,
				'url' => thegig_remove_protocol_from_url( THEGIG_THEME_URL )
				)
			);
