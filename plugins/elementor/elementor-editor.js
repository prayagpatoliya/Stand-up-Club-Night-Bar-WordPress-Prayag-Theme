...
jQuery( document ).ready(
	function() {
		...
		thegig_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="thegig_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.thegig_post_editor' );
						if ( link.length > 0 ) {
							thegig_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.thegig_post_editor', function(e) {
						thegig_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('thegig_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function thegig_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
