...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			thegig_create_observer( 'thegig-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				thegig_gutenberg_editor_init();
			} );
		} else {
		 	thegig_gutenberg_editor_init();
		}
	...
	function thegig_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'thegig_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'thegig_inited' ) ) {
	...
	// Init on page load
	function thegig_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = thegig_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + THEGIG_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + THEGIG_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in THEGIG_STORAGE['override_classes'] ) {
				$obj.addClass( THEGIG_STORAGE['override_classes'][i].replace( '%s', THEGIG_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( thegig_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && THEGIG_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'thegig_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			thegig_remove_observer( 'thegig-check-editor-styles-wrapper' );
			thegig_create_observer( 'thegig-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'thegig_inited' );
	}
} );
